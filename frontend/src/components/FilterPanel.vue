<template>
  <aside class="filter-panel" :class="{ open: modelValue }">
    ...
    <!-- ── Host (searchable + scrollable) ── -->
    <section class="filter-section flex-section" :style="flexStyle(availableHosts.length)">
    ...
    <!-- ── Message Search (pinned at bottom) ─────── -->
...
const emit = defineEmits([
  ...
  'update:messageSearch','shift','reset','toggle-live','exit-live',
])
...
const filteredTags = computed(() =>
  tagSearch.value
    ? props.availableTags.filter(t => t.toLowerCase().includes(tagSearch.value.toLowerCase()))
    : props.availableTags
)
...
// Always use full names regardless of what the API returns as label.
...
function sevBadgeClass(val) {
  ...
  return [
    'sev-' + val,
    state === 'include' ? 'sev-include' : '',
    state === 'exclude' ? 'sev-exclude' : '',
    state === 'neutral' ? 'sev-neutral' : '',
  ]
}

// ── Facility badge display ────────────────────────────────────────────────────
function facBadgeClass(val) {
  const state = getState(props.facilities, props.excludeFacilities, val)
  return [
    state === 'include' ? 'fac-include' : '',
    state === 'exclude' ? 'fac-exclude' : '',
    state === 'neutral' ? 'fac-neutral' : '',
  ]
}

function tagBadgeClass(val) {
  const state = getState(props.tags, props.excludeTags, val)
  return [
    state === 'include' ? 'fac-include' : '',
    state === 'exclude' ? 'fac-exclude' : '',
    state === 'neutral' ? 'fac-neutral' : '',
...
// ── List state display (Facility / Host / Tag) ────────────────────────────────
...
function listClass(type, val) {
  ...
  const state = getState(inc, exc, val)
  if (state === 'include') return 'list-include'
  if (state === 'exclude') return 'list-exclude'
  return ''
}
...
function listStateClass(type, val) {
  const [inc, exc] = getListArrays(type)
  const state = getState(inc, exc, val)
  if (state === 'include') return 'state-inc'
  if (state === 'exclude') return 'state-exc'
  return 'state-neu'
}
...
// ── Section height ────────────────────────────────────────────────────────────
// Sections never grow beyond their content. ROW_H ≈ list-item height.
const ROW_H    = 29
const CHROME_H = 80  // label + search input + padding + gaps + border
...
// ── Rolling "now" for To field in live mode ───────────────────────────────────
const liveNow = ref('')
let liveTimer = null
...
.filter-panel {
  ...
  height: 100%;        /* fill the logs-body height exactly */
...
/* Fixed-height sections: never grow or shrink */
.fixed-section {
  flex-shrink: 0;
}

/* Flexible sections: share the remaining height equally, each scrolls internally */
.flex-section {
  flex: 1;
  min-height: 0;       /* crucial — allows shrinking below content size */
}

/* Message search: no top border */
.message-section {
  flex-shrink: 0;
  border-top: none;
  border-bottom: none;
...
/* ── Facility badges — match dur-btn / shift-btn style ── */
.fac-badge-btn {
...
.fac-badge-btn.fac-include {
  background: var(--color-primary); color: #fff;
  border-color: var(--color-primary);
}
.fac-badge-btn.fac-exclude {
  color: #dc2626;
  border-color: #dc2626;
  text-decoration: line-through;
}

/* Tag list: no scroll, full height */
.tag-list-full {
  overflow: visible;
  max-height: none;
  border: 1px solid var(--border);
...
/* ── Severity badges ────────────────────────── */
/* Same visual style as the table severity badges, with 3-state overlay */
.sev-row {
...
/* Neutral: faded, no border */
.sev-badge-btn.sev-neutral { opacity: .35; }
.sev-badge-btn.sev-neutral:hover { opacity: .7; }

/* Include: full opacity + white border glow */
.sev-badge-btn.sev-include {
  opacity: 1;
  border-color: rgba(255,255,255,.6);
  box-shadow: 0 0 0 1px rgba(255,255,255,.25);
}

/* Exclude: desaturated + red diagonal overlay via outline */
.sev-badge-btn.sev-exclude {
  opacity: .55;
  border-color: #dc2626;
  text-decoration: line-through;
}

/* Severity colors — match LogTable */
...
/* ── Compact list (Facility / Host / Tag) ───── */
.list-search {
  width: 100%; padding: .28rem .45rem; box-sizing: border-box;
  border: 1px solid var(--border); border-radius: var(--radius);
  background: var(--bg); color: var(--text); font-size: .76rem;
  flex-shrink: 0;
}
.list-search:focus { outline: 2px solid var(--color-primary); outline-offset: -1px; }

/* The list fills remaining height of its flex-section and scrolls */
.filter-list {
  flex: 1;
  overflow-y: auto;
  min-height: 0;
